...
final public class Loaf {
    ...
    public enum Icon {
        public static let success = TipsIcons.imageOfSuccess().withRenderingMode(.alwaysTemplate)
        public static let error = TipsIcons.imageOfError().withRenderingMode(.alwaysTemplate)
        public static let warning = TipsIcons.imageOfWarning().withRenderingMode(.alwaysTemplate)
        public static let info = TipsIcons.imageOfInfo().withRenderingMode(.alwaysTemplate)
    }
...
final class LoafViewController: UIViewController {
    ...
    init(_ toast: Loaf) {
        self.loaf = toast
        self.transDelegate = TostManager(loaf: toast, size: .zero)
        ...
        let height = max(toast.message.heightWithConstrainedWidth(width: 240, font: font) + 12, 30)
        //MARK: 获取文字宽度
        let cWidth = toast.message.boundingRect(with:CGSize(width:CGFloat(MAXFLOAT), height: height), options: .usesLineFragmentOrigin, attributes: [.font:font], context:nil).size.width
        print(cWidth)
        preferredContentSize = CGSize(width: cWidth + 50, height: height)
    }
